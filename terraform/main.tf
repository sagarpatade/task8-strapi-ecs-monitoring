...
resource "aws_cloudwatch_log_group" "strapi_logs" {
  name              = "/ecs/strapi-task8-v15"
  retention_in_days = 7
}

# 3. Security Group
resource "aws_security_group" "strapi_sg" {
  name        = "strapi-sg-task8-v15"
...
resource "aws_ecs_cluster" "main" {
  name = "strapi-cluster-task8-v15"
}

# 5. Task Definition
resource "aws_ecs_task_definition" "app" {
  family                   = "strapi-task-v15"
  network_mode             = "awsvpc"
  ...
  container_definitions = jsonencode([{
    ...
    logConfiguration = {
      logDriver = "awslogs"
      options = {
        "awslogs-group"         = "/ecs/strapi-task8-v15"
